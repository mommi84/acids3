...
public abstract class WeightedEditDistanceFilter {
	...
	public WeightedEditDistanceFilter() {
		...
		Scanner in = null;
		try {
			in = new Scanner(new File("etc/confusion-matrix.txt"));
		} catch (FileNotFoundException e) {
			System.err.println("Missing file `./etc/confusion-matrix.txt`!");
			return;
		}
		for(char c1='a'; c1<='{'; c1++) {
			for(char c2='a'; c2<='{'; c2++) {
